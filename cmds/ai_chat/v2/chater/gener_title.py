...
async def gener_title(history: list, ctx: commands.Context, length: int = 15) -> str:
    try:
        client = Chater(ctx, model='ai-local:gemma4-12b')
        client.change_system_prompt(gener_title_prompt)
        ...
        if not return_item:
            for item in reversed(history):
                if item.get('role') == 'user':
                    return_item = item.get('content', 'no_title').strip()[:length]
                    break
