...
class AIChannelTwo(Cog_Extension):
    def __init__(self, bot):
        super().__init__(bot)
        self.db = MongoDB_DB.aichannel_chat_history
        ...
        try:
            async with ctx.typing():
                ...
                if not (await collection.find_one({'_id': 'ai_channel_setting'})):
                    return await ctx.send(await get_translate('cancel_ai_channel_channel_not_found', ctx))
                
                '''i18n'''
                button_check_text = await get_translate('button_cancel_ai_channel_check', ctx)
                button_refuse_text = await get_translate('button_cancel_ai_channel_refuse', ctx)
                eb = load_translated(await get_translate('embed_cancel_ai_channel_whether_cancel', ctx))[0]
        ...
        try:
            async with ctx.typing():
                ...
                if not (await check_model()):
                    return await ctx.send((await get_translate('send_change_ai_channel_model_not_available_model', ctx)).format(model=model))

                if not (await collection.find_one({'_id': 'ai_channel_setting'})):
                    return await ctx.send(await get_translate('send_change_ai_channel_model_channel_not_found', ctx))
    ...
    async def change_ai_channel_system_prompt(self, ctx: commands.Context, system_prompt: str):
        try:
            async with ctx.typing():
                ...
                if not (await collection.find_one({'_id': 'ai_channel_setting'})):
                    return await ctx.send(await get_translate('send_change_ai_channel_system_prompt_channel_not_found', ctx))
    ...
    async def set_chat_human(self, ctx: commands.Context):
        try:
            async with ctx.typing():
                ...
                if channel_exist:
                    return await ctx.send(await get_translate('send_set_chat_human_channel_exist', ctx))
        ...
        try:
            async with ctx.typing():
                ...
                if not (await collection.find_one({'_id': 'chat_human_setting'})):
                    return await ctx.send(await get_translate('cancel_chat_human_channel_not_found', ctx))
                
                '''i18n'''
                button_check_text = await get_translate('button_cancel_chat_human_check', ctx)
                button_refuse_text = await get_translate('button_cancel_chat_human_refuse', ctx)
                eb = load_translated(await get_translate('embed_cancel_chat_human_whether_cancel', ctx))[0]
                eb_title = eb.get('title')
                ''''''
