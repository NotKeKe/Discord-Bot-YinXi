...
class Player:
    ...
    def session_id(self, session_id: str):
        self.add_passed_session_id(self._session_id)
        self._session_id = session_id

    def add_passed_session_id(self, session_id: str):
        '''將 session id 加入進 passed_session_ids'''
        self.passed_session_ids.add(session_id)
        
    def update_state(self, title: str | None, audio_url: str, srts: dict[str, list], duration: int, current_time: int, is_paused: bool):
        if title:
            self.title = title
    ...
    def get_state(self, lang: str = '') -> dict:
        '''取得給 /check_song 的狀態'''
        target_srt = ''
        if lang:
        ...
        return {
            ...
            "title": self.title,
            "audio_url": f'/player/stream?token={get_token(self.audio_url, self.guild_id)}&guild_id={self.guild_id}&session_id={self.session_id}',
...
def is_player_exist(guild_id: str, uuid: str = '', session_id: str = '') -> bool:
    player = players.get(int(guild_id))
    if not player:
        return False
    if uuid and player.uuid != uuid:
        return False
    if session_id and player.session_id != session_id and session_id not in player.passed_session_ids:
        return False
    return True
